...
def invoke(path, output_path):
    for img, img_path in traverse_path_for_image(path):
        ...
        # write_boxes_result(formatted_ocr_result, os.path.join(output_path, image_name + '-1' + '.txt'))
...
def reduce_ocr_result_to_txt_task(ocr_result):
  ...
  for segment_result in ocr_result:
    line = segment_coords_reducer(segment_result) + segment_recognition_reducer(segment_result)
    result.append(line)
